...
export default function AlbumCard({ album, songCount, onFilterByAlbum, isActive }: AlbumCardProps) {
  return (
    <div
      ...
      className={`group relative overflow-hidden rounded-2xl border p-4 cursor-pointer transition-all duration-300 transform hover:-translate-y-1 select-none flex gap-4 items-center ${
        isActive 
          ? 'bg-gradient-to-br from-violet-950/40 via-[#131b2f] to-[#0c0f18] border-violet-500/50 shadow-md shadow-violet-500/5' 
          : 'bg-[#111625]/40 border-white/[0.04] hover:bg-[#151c30]/70 hover:border-white/10'
      }`}
    >
      {/* Decorative gradient color flash based on album custom theme */}
      <div className={`absolute -right-10 -bottom-10 w-24 h-24 rounded-full bg-gradient-to-br ${album.accentColor} opacity-5 group-hover:opacity-15 blur-xl transition-opacity pointer-events-none`} />
      ...
      <div className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-xl overflow-hidden shadow-md flex-shrink-0 bg-neutral-900">
        ...
        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
          <div className="p-2 rounded-full bg-violet-600 text-white shadow-lg">
